...
export  const Game = ()=>{
    ...
    useEffect(()=> {
        ...
        if(gyroscope.alpha > 150){
            context.drawImage(image, canvas.width - (gyroscope.alpha - 250) * 7, -9 * gyroscope.beta + canvas.height/2, 50, 50)
        }else{
            context.drawImage(image,-9* gyroscope.alpha + canvas.width/2, -9 * gyroscope.beta + canvas.height/2, 50, 50)
        }
