...
export const Game = ()=>{
    ...
    useEffect(()=>{
        ...
        SocketIO.socket.on('Shooted', (data:IGyroscope)=>{
            console.log('Shooted on')
            console.log(data)
            setShooted({x: Math.floor(AlphaToX(data.alpha)), y: BetaToY(data.beta)})
        })
    ...
    return (<> 
    {isConnected ? <canvas onClick={()=>{

    }}

    ref={canvasRef} id='canvas' className={`w-screen h-screen bg-[url('https://img.itch.zone/aW1nLzI1OTYzMTcuZ2lm/original/C4h3cc.gif')] bg-cover`}/> 
    : (<>
